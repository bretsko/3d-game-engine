...
typedef struct {
  ...
  vector<int> material_ids; // per-face material ID
  vector<tag_t> tags;       // SubD tag
} mesh_t;
...
public:
  MaterialFileReader(const string &mtl_basepath)
      : m_mtlBasePath(mtl_basepath) {}
  virtual ~MaterialFileReader() {}
  virtual bool operator()(const string &matId,
                          vector<material_t> &materials,
                          map<string, int> &matMap, string &err);

private:
  string m_mtlBasePath;
};
...
/// Loads materials into map
void LoadMtl(map<string, int> &material_map, // [output]
             vector<material_t> &materials,       // [output]
...
std::istream& safeGetline(std::istream& is, string& t)
{
    t.clear();
...
static bool exportFaceGroupToShape(
    ...
    vector<tag_t> &tags, const int material_id, const string &name,
    bool clearCache, unsigned int flags, string& err ) {
  if (faceGroup.empty()) {
    return false;
  }
   ...
   if (nIndexs % 3 == 0) {
    ...
    for (register size_t iIndices = 0; iIndices < nIndexs; iIndices += 3) {
     ...
     memcpy(&shape.mesh.normals[shape.mesh.indices[iIndices + 2] * 3], &normal, sizeof(float3));
          ...
          std::stringstream ss;
    ss << "WARN: The shape " << name << " does not have a topology of triangles, therfore the normals calculation could not be performed. Select the tinyobj::triangulation flag for this object." << endl;
    err += ss.str();
   }
  }
...
void LoadMtl(map<string, int> &material_map,
             vector<material_t> &materials, std::istream &inStream) {
  ...
  while (inStream.peek() != -1) {
    ...
    if ((0 == strncmp(token, "newmtl", 6)) && IS_SPACE((token[6]))) {
      ...
      if (!material.name.empty()) {
        material_map.insert(pair<string, int>(
...
bool MaterialFileReader::operator()(const string &matId,
  ...
  if (!m_mtlBasePath.empty()) {
    filepath = string(m_mtlBasePath) + matId;
  } else {
    filepath = matId;
  }
...
bool LoadObj(vector<shape_t> &shapes,       // [output]
             vector<material_t> &materials, // [output]
             string &err, const char *filename, const char *mtl_basepath,
             unsigned int flags) {
  ...
  if (!ifs) {
    errss << "Cannot open file [" << filename << "]" << endl;
    err = errss.str();
    return false;
  }
...
bool LoadObj(vector<shape_t> &shapes,       // [output]
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'f' && IS_SPACE((token[1]))) {
      ...
      faceGroup.push_back(vector<vertex_index>());
